...
# TODO: explain why we need such complex codes for node
...
class Spec2MolDenoisingDiffusion(pl.LightningModule):
    ...
    def _mcts_select(self, root: MctsNode, c_puct: float) -> Tuple[list, MctsNode]:
        ...
        for path, leaf in leaves:
            if leaf is root:
                # root as a leaf at the beginning
                total_N = max(1, root.N)
                # UCT: V + c * sqrt(ln N(parent) / N(leaf))
                parent_N = max(1, root.N)
                leaf_U = c_puct * math.sqrt(max(0.0, math.log(parent_N)) / max(1, leaf.N))
                leaf.U = leaf_U
                score = leaf.V + leaf.U
            else:
                parent = leaf.parent
                parent_N = max(1, parent.N)
                leaf.U = c_puct * math.sqrt(max(0.0, math.log(parent_N)) / max(1, leaf.N))
                score = leaf.V + leaf.U
            if score > best_score:
                best_score = score
                best = (path, leaf)
    ...
    def _mcts_evaluate(self, node: MctsNode, env_meta: dict, spectra: np.ndarray) -> float:
        ...
        if t_int == 0:
            ...
            e_idx = torch.argmax(pred_E, dim=-1)
            E_hat = F.one_hot(e_idx, num_classes=self.Edim_output).float()
            valid, smi, mol = self._terminal_check_and_smiles(state['X_t'][0], E_hat[0])
            if not valid:
                return 0.0
            self._ensure_verifier()
            if smi in self._smiles_score_cache:
